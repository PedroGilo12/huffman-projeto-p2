...
static int get_file_names(char* input_file_name, char* output_file_name);
...
static int extract_interface(char* input_file_name, char* output_file_name)
{
    ...
    get_header_from_file(input_file_name, &header);
    ...
    huffman_output->linkedList =
        create_tree_from_preorder(header->preorder_tree, &index, &qtd_caracter_scape);
    ...
    extract_file(input_file_name, output_file_name, huffman_output, qtd_caracter_scape);
    return 0;
}