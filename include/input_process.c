...
huffman_tree_t *make_huffman_tree(linked_list_t **linked_list)
{
   ...
   huffman_tree->preorder = (unsigned char *)malloc(huffman_tree->tree_size * sizeof(unsigned char));
                            ...
                            unsigned char **pre_order_tree, int binary_word)
{
    ...
    * ordem. */
   static unsigned long dictionary_index = 0;
   static unsigned long pre_order_index = 0;
   ...
   unsigned char *byte = &((byte_frequency_t *)((*linked_list)->data))->byte;

   /* Adiciona o nó à string em pré ordem e soma +1 no índice. */
   (*pre_order_tree)[pre_order_index] = *byte;
   pre_order_index += 1;

   /* Preferência para os filhos à esquerda, se não forem NULL. Prosseguir. */
   ...
   /* Se ambos os filhos, da esquerda e da direita forem NULL, então chegou em uma folha,
         * deve adicionar essa folha ao dicionário. */
   if (((*linked_list)->right == NULL) && ((*linked_list)->left == NULL)) {
       /* Aloca memória para o item do dicionário. */
       unsigned long *dictionary_item =
           (unsigned long *)malloc(3 * sizeof(unsigned long));

       /* Atribui valores ao item. */
       dictionary_item[0] = (unsigned long)(*linked_list)->data->byte;
       dictionary_item[1] = binary_word;
       dictionary_item[2] = depth;

       /* Inclui o item no dicionário. */
       (*dictionary)[dictionary_index] = dictionary_item;
       dictionary_index++;

       /* Casos especiais: Adição do caractere de escape à string em pré ordem. */
       if ((*linked_list)->data->byte == '*' || (*linked_list)->data->byte == '\\') {
           /* Aumenta em 1 byte a memória atual para a string em pré ordem. */
           unsigned char *new_pre_order_tree =
               (unsigned char *)malloc(pre_order_index + sizeof(unsigned char));
           memcpy(new_pre_order_tree, *pre_order_tree, pre_order_index);
...
#if DEBUG_MODE
       printf("%c , 0x%x, depth: %lu\n", (*linked_list)->data->byte, binary_word,
              depth);
#endif
   }
}

static unsigned char *make_header(huffman_tree_t *huffman_tree)
{
   /* Verificação de ponteiro. */
                  ...
                  huffman_tree_t *huffman_tree)
{
   ...
   while (1) {
       ...
       for (int scan_dict = 0; scan_dict <= huffman_tree->tree_size / 2; scan_dict++) {
           ...
           unsigned char target_byte = (unsigned char)huffman_tree->dictionary[scan_dict][0];
   ...
   insert_header(output_file_name, header, 2 + huffman_tree->tree_size);
